select
    ...
    coalesce(count(case when te.status = :statusDraft then 1 end), 0) as "DraftCount",
    coalesce(count(case when te.status = :statusApproved then 1 end), 0) as "ApprovedCount",
    coalesce(count(case when te.status = :statusRejected then 1 end), 0) as "RejectedCount"
