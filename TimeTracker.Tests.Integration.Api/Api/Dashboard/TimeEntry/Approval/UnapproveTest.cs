...
public class UnapproveTest : BaseTest
{
    private readonly string Url = "/dashboard/time-entry/approval/unapprove";
    ...
    public UnapproveTest(ApiCustomWebApplicationFactory factory) : base(factory)
    {
    ...
    {
        var response = await PostRequestAsAnonymousAsync(Url, new UnapproveRequest
        {
    ...
    [Fact]
    public async Task ShouldRevertApprovedToDraft()
    {
        ...
        var response = await PostRequestAsync(Url, _ownerJwtToken, new UnapproveRequest
        {
        ...
        Assert.Equal(TimeEntryStatus.Draft, actual.Status);
    }
}
