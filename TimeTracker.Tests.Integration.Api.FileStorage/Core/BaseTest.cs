...
namespace TimeTracker.Tests.Integration.Api.FileStorage.Core;
...
public class BaseTest: IClassFixture<ApiCustomWebApplicationFactory>, IDisposable
{
    ...
    public async Task<HttpResponseMessage> GetRequestAsync(string url, FileStorageAccessKeyEntity? accessKey = null, Dictionary<string, string>? urlParams = null)
    {
        await CommitDbChanges();
    ...
    public async Task<HttpResponseMessage> PostMultipartFormDataRequestAsync(
        string url,
        FileStorageAccessKeyEntity? accessKey = null,
